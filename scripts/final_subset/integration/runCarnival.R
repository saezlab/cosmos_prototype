...
CARNIVAL_Result <- runCARNIVAL(CplexPath="~/Documents/cplex",
                               Result_dir="results/multi_omic/final_sample/carnival/",
                               CARNIVAL_example=NULL,
                               UP2GS=F,
                               netFile = "support/meta_network_with_X_nobadchar.tsv", 
                               measFile = "results/metabolomic/final_samples/metab_input_carnival.tsv", 
                               inputFile = "results/phospho/final_samples/signaling_input_carnival.tsv",
                               weightFile = NULL, 
                               timelimit = 7200,
                               mipGAP = 0.12
...
CARNIVAL_Result2 <- runCARNIVAL(CplexPath="~/Documents/cplex",
                               ...
                               UP2GS=F,
                               netFile = "support/meta_network_with_X_nobadchar.tsv", 
                               measFile = "results/phospho/final_samples/signaling_input_carnival.tsv", 
                               inputFile = "results/metabolomic/final_samples/metab_input_carnival.tsv",
                               weightFile = NULL, 
                               timelimit = 7200
)# 2.44

signaling_input_carnival <- as.data.frame(
  read_delim("results/phospho/final_samples/signaling_input_carnival.tsv", 
             "\t", escape_double = FALSE, trim_ws = TRUE))

metab_input_carnival <- as.data.frame(
  read_delim("results/metabolomic/final_samples/metab_input_carnival.tsv", 
                                   "\t", escape_double = FALSE, trim_ws = TRUE))
...
write_csv(sif, "results/multi_omic/final_sample/carnival/subnet_sif_full_wTF.csv")

write_csv(att, "results/multi_omic/final_sample/carnival/subnet_sif_full_att_wTF.csv")

write_csv(sif2, "results/multi_omic/final_sample/carnival/subnet_sif2_full_wTF.csv")

write_csv(att2, "results/multi_omic/final_sample/carnival/subnet_sif_full_att2_wTF.csv")

sif3 <- as.data.frame(rbind(sif[as.numeric(as.character(sif$Weight)) >= 80,], sif2[as.numeric(as.character(sif2$Weight)) >= 80,]))
write_csv(sif3, "results/multi_omic/final_sample/carnival/subnet_sif3_wTF.csv")

save.image("results/multi_omic/final_sample/carnival/results_CARNIVAL.Rdata")

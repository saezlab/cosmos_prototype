...
# TPR$TPR <- TPR$TPR*max(TPR$n)
# ggplot(TPR) +
#   geom_line(aes(x = threshold, y = TPR, group = group, color = group)) +
#   geom_bar(aes(x = threshold, y = n, group = group, fill = group), stat = "identity", position="dodge", alpha = 0.5) + 
#   scale_y_continuous(sec.axis = sec_axis(~./max(TPR$n), name = "TPR")) +
#   ylab("n") +
#   theme_minimal()

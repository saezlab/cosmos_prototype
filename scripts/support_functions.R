...
#'\code{createLinearColors}
#'
#'This function is designed to generate a vector of colors in hexadecimal code for
#'use with ggplot from a vector of numbers. In particular, white will be centred on 0.
#'
#'@param numbers a vector of numbers to be color coded
#'@param withZero wether or not the vector has both positive and negative values
#'@param maximum obsolete, need to be removed
#'
#'@return a vector of colors in hexadecimal code for
#'use with ggplot from a vector of numbers.
...
createLinearColors <- function(numbers, withZero = T, maximum = 100)
{
  ...
  else
  {
    if (max(numbers, na.rm = T) < 0)
    {
      ...
      myPalette <- colorRampPalette(c("royalblue3","white"))
      myColors <- myPalette(maximum)
    }
    else
    {
